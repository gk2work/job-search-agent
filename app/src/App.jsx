...
// ── Job age color helper ──
function getAgeColor(posted) {
  if (!posted || posted === "Recent") return "#3a4660";
  const t = posted.toLowerCase();
  if (t.includes("today") || t.includes("just now") || /\bhour/.test(t) || t.includes("1d") || t.includes("1 day")) return "#10b981";
  if (t.includes("2d") || t.includes("3d") || t.includes("2 day") || t.includes("3 day")) return "#10b981";
  if (t.includes("4d") || t.includes("5d") || t.includes("6d") || t.includes("7d") || t.includes("1 week") || t.includes("1w")) return "#eab308";
  if (t.includes("2w") || t.includes("2 week") || t.includes("14d")) return "#f97316";
  if (t.includes("3w") || t.includes("4w") || t.includes("month") || t.includes("30d")) return "#ef4444";
  return "#3a4660";
}

// ── Cover Letter Preview Modal ──
function CoverLetterModal({ job, initialCL, onConfirm, onCancel }) {
  ...
  return (
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,.75)", zIndex:1000, display:"flex", alignItems:"center", justifyContent:"center", padding:16 }}>
      <div style={{ background:"#090d1a", border:"1px solid #1e293b", borderRadius:12, padding:20, maxWidth:500, width:"100%", maxHeight:"85vh", display:"flex", flexDirection:"column", boxShadow:"0 8px 40px #0008" }}>
        <div style={{ fontSize:13, fontWeight:800, color:"#e2e8f0", fontFamily:"var(--display)", marginBottom:3 }}>Cover Letter Preview</div>
        <div style={{ fontSize:9.5, color:"#3a4660", fontFamily:"var(--mono)", marginBottom:10 }}>{job.title} @ {job.company}</div>
        <textarea
          value={cl} onChange={e => setCl(e.target.value)}
          style={{ flex:1, minHeight:200, maxHeight:320, background:"#060810", border:"1px solid #1e293b", color:"#c4b5fd",
            borderRadius:6, padding:10, fontSize:11, fontFamily:"var(--sans)", lineHeight:1.6, resize:"vertical" }}
        />
        <div style={{ display:"flex", gap:8, marginTop:12 }}>
          <button onClick={() => onConfirm(cl)} style={{
            flex:1, padding:"8px", fontSize:11, fontWeight:700, borderRadius:7,
            background:"linear-gradient(135deg,#6366f1,#a855f7)", color:"#fff", border:"none", cursor:"pointer"
          }}>Apply with this Letter</button>
          <button onClick={onCancel} style={{
            padding:"8px 16px", fontSize:11, borderRadius:7,
            background:"#1e293b", color:"#64748b", border:"1px solid #2d3a54", cursor:"pointer"
          }}>Cancel</button>
        </div>
      </div>
    </div>
...
export default function App() {
  ...
  const [results, setResults] = useState(() => {
    try { return JSON.parse(localStorage.getItem("vj_results") || "[]"); } catch { return []; }
  });
  const [searching, setSearching] = useState(false);
  const [logs, setLogs] = useState([]);
  const [saved, setSaved] = useState(() => {
    try { return new Set(JSON.parse(localStorage.getItem("vj_saved") || "[]")); } catch { return new Set(); }
  });
  const [allResults, setAllResults] = useState(() => {
    try { return JSON.parse(localStorage.getItem("vj_all_results") || "[]"); } catch { return []; }
  });
  ...
  // ── Persist results & saved set to localStorage whenever they change ──
  useEffect(() => {
    localStorage.setItem("vj_results", JSON.stringify(results));
  }, [results]);

  useEffect(() => {
    localStorage.setItem("vj_all_results", JSON.stringify(allResults));
  }, [allResults]);

  useEffect(() => {
    localStorage.setItem("vj_saved", JSON.stringify([...saved]));
  }, [saved]);

  // ── Profile editor (editable CV fields sent with apply) ──
  const [profileEdit, setProfileEdit] = useState(() => {
    try { return JSON.parse(localStorage.getItem("vj_profile") || "{}"); } catch { return {}; }
  });
  const saveProfile = (update) => {
    const merged = { ...profileEdit, ...update };
    setProfileEdit(merged);
    localStorage.setItem("vj_profile", JSON.stringify(merged));
  };

  // ── Per-job notes ──
  const [notes, setNotes] = useState(() => {
    try { return JSON.parse(localStorage.getItem("vj_notes") || "{}"); } catch { return {}; }
  });
  const [editNoteKey, setEditNoteKey] = useState(null);
  const saveNote = (key, text) => {
    const n = { ...notes, [key]: text };
    setNotes(n);
    localStorage.setItem("vj_notes", JSON.stringify(n));
  };

  // ── Cover-letter preview modal ──
  const [clModal, setClModal] = useState(null); // { job, coverLetter, resolve }
  ...
  // ── Apply filters (smart auto-apply) ──
  const [applyMinMatch, setApplyMinMatch] = useState(70);

  // ── Multiple CV profiles ──
  const [cvProfiles, setCvProfiles] = useState(() => {
    try { return JSON.parse(localStorage.getItem("vj_cv_profiles") || "[]"); } catch { return []; }
  });
  ...
  useEffect(() => {
    ...
    check();
    const t = setInterval(check, 15000); // re-check every 15 s
  ...
  // Show cover-letter modal and return the (possibly edited) letter
  const previewCoverLetter = useCallback((job, cl) => {
    return new Promise(resolve => {
      setClModal({ job, coverLetter: cl, resolve });
    });
  }, []);

  // Auto-apply via backend
  const applyJob = useCallback(async (job, skipPreview = false) => {
    const key = job.url || (job.title + job.company);

    // Duplicate guard
    const already = applyLog.find(a => a.url === job.url && a.status === "applied");
    if (already) {
      if (!window.confirm(`Already applied to "${job.title}" on ${new Date(already.appliedAt).toLocaleDateString()}. Apply again?`)) return;
    }
    ...
    try {
      ...
      if (!skipPreview && apiKey) {
        ...
        if (clResp?.coverLetter) {
          prebuiltCL = await previewCoverLetter(job, clResp.coverLetter);
          if (prebuiltCL === null) {
            // User cancelled
            setApplyStatus(prev => { const n = { ...prev }; delete n[key]; return n; });
            return;
          }
        }
      }

      // Build override CV from local profile edits
  ...
  }, [apiKey, backendOnline, applyLog, profileEdit, previewCoverLetter]);
  ...
  const applyAll = useCallback(async () => {
    ...
    const pending = results.filter(j => {
      ...
      return (!s || s === "error") && (j.match || 0) >= applyMinMatch;
    });
    if (!pending.length) { alert(`No jobs with match ≥ ${applyMinMatch}% to apply to.`); return; }
  ...
  const searchOne = useCallback(async (kw) => {
    ...
    if (unique.length > 0) {
      ...
      // Summary card
      const byPlatform = {};
      unique.forEach(j => { byPlatform[j.source] = (byPlatform[j.source] || 0) + 1; });
      setSearchSummary({ total: unique.length, highMatch: unique.filter(j => j.match >= 85).length, byPlatform, keyword: kw });
    ...
    <div style={{ fontFamily:"var(--sans)", background:"#060810", color:"#e2e8f0", minHeight:"100vh" }}>
      ...
      <div style={{ padding:"10px 14px", maxHeight:"calc(100vh - 110px)", overflowY:"auto" }}>
        ...
        {tab === "search" && (<div>
          ...
          {/* Post-search summary card */}
          {searchSummary && !searching && (
            <div style={{ background:"#090d1a", border:"1px solid #6366f125", borderRadius:8, padding:"10px 12px", marginTop:8, marginBottom:2 }}>
              <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:6 }}>
                <span style={{ fontSize:9.5, fontWeight:700, color:"#818cf8", fontFamily:"var(--mono)" }}>SEARCH SUMMARY — "{searchSummary.keyword}"</span>
                <button onClick={() => setSearchSummary(null)} style={{ background:"none", border:"none", color:"#1e293b", cursor:"pointer", fontSize:11 }}>✕</button>
              </div>
              <div style={{ display:"flex", gap:6, flexWrap:"wrap" }}>
                <div style={{ background:"#6366f112", borderRadius:5, padding:"4px 10px", textAlign:"center" }}>
                  <div style={{ fontSize:15, fontWeight:800, color:"#818cf8", fontFamily:"var(--mono)" }}>{searchSummary.total}</div>
                  <div style={{ fontSize:7.5, color:"#3a4660", fontWeight:600 }}>Total</div>
                </div>
                <div style={{ background:"#10b98112", borderRadius:5, padding:"4px 10px", textAlign:"center" }}>
                  <div style={{ fontSize:15, fontWeight:800, color:"#10b981", fontFamily:"var(--mono)" }}>{searchSummary.highMatch}</div>
                  <div style={{ fontSize:7.5, color:"#3a4660", fontWeight:600 }}>85%+ Match</div>
                </div>
                {Object.entries(searchSummary.byPlatform).map(([src, cnt]) => (
                  <div key={src} style={{ background:"#ffffff06", borderRadius:5, padding:"4px 10px", textAlign:"center" }}>
                    <div style={{ fontSize:15, fontWeight:800, color:"#c4b5fd", fontFamily:"var(--mono)" }}>{cnt}</div>
                    <div style={{ fontSize:7.5, color:"#3a4660", fontWeight:600 }}>{src}</div>
                  </div>
                ))}
              </div>
            </div>
          ...
          {results.length > 0 && (<div style={{ marginTop:8 }}>
            <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:6 }}>
              <div style={{ display:"flex", alignItems:"center", gap:6 }}>
                ...
                <span style={{ fontSize:8, color:"#10b981", fontFamily:"var(--mono)", background:"#10b98112", padding:"1px 6px", borderRadius:3 }}>saved locally</span>
              ...
              <div style={{ display:"flex", gap:4, alignItems:"center" }}>
                {backendOnline && (<>
                  <select value={applyMinMatch} onChange={e => setApplyMinMatch(+e.target.value)} style={{
                    fontSize:8.5, fontWeight:700, padding:"3px 6px", borderRadius:4,
                    background:"#060810", border:"1px solid #1e293b", color:"#818cf8", fontFamily:"var(--mono)"
                  }}>
                    {[0,60,70,75,80,85,90].map(v => <option key={v} value={v}>{v}%+ only</option>)}
                  </select>
                  <button className="hov" onClick={applyAll} disabled={searching} style={{
            ...
            {results.map((j,idx)=>(<JobRow key={idx} job={j} idx={idx} saved={saved} toggleSave={toggleSave} applyStatus={applyStatus} onApply={applyJob} backendOnline={backendOnline} notes={notes} onNoteChange={saveNote} editNoteKey={editNoteKey} setEditNoteKey={setEditNoteKey} />))}
          </div>)}
        </div>)}
          ...
          {allResults.length === 0 ? (
            ...
            <div style={{ display:"flex", gap:4, alignItems:"center", marginBottom:8, flexWrap:"wrap" }}>
              <button onClick={() => setShowFilters(p => !p)} style={{
                fontSize:8.5, fontWeight:700, padding:"4px 10px", borderRadius:5,
                background: showFilters ? "#6366f115" : "#090d1a", color: showFilters ? "#818cf8" : "#3a4660",
                border:"1px solid " + (showFilters ? "#6366f130" : "#111827"), fontFamily:"var(--mono)"
              }}>{showFilters ? "▲ Filters" : "▼ Filters"}</button>
              {showFilters && (<>
                <select value={filterMinMatch} onChange={e => setFilterMinMatch(+e.target.value)} style={{
                  fontSize:8.5, padding:"4px 7px", borderRadius:5, background:"#060810", border:"1px solid #1e293b", color:"#e2e8f0", fontFamily:"var(--mono)"
                }}>
                  {[0,60,70,80,85,90].map(v => <option key={v} value={v}>{v}%+ match</option>)}
                </select>
                <select value={filterSource} onChange={e => setFilterSource(e.target.value)} style={{
                  fontSize:8.5, padding:"4px 7px", borderRadius:5, background:"#060810", border:"1px solid #1e293b", color:"#e2e8f0", fontFamily:"var(--mono)"
                }}>
                  <option value="all">All sources</option>
                  {["LinkedIn","Naukri","Indeed","Glassdoor","Google Jobs"].map(s => <option key={s} value={s}>{s}</option>)}
                </select>
                <input value={filterKw} onChange={e => setFilterKw(e.target.value)} placeholder="Filter title/company…"
                  style={{ fontSize:8.5, padding:"4px 8px", borderRadius:5, background:"#060810", border:"1px solid #1e293b", color:"#e2e8f0", fontFamily:"var(--mono)", width:160 }} />
                {(filterMinMatch > 0 || filterSource !== "all" || filterKw) &&
                  <button onClick={() => { setFilterMinMatch(0); setFilterSource("all"); setFilterKw(""); }} style={{
              ...
              <div style={{ display:"flex", gap:6 }}>
                {(() => {
                  const filtered = allResults.filter(j =>
                    (j.match||0) >= filterMinMatch &&
                    (filterSource === "all" || j.source === filterSource) &&
                    (!filterKw || (j.title+j.company).toLowerCase().includes(filterKw.toLowerCase()))
                  );
                  return [{v:filtered.length,l:"Shown",c:"#6366f1"},{v:filtered.filter(j=>j.match>=85).length,l:"85%+",c:"#10b981"},{v:saved.size,l:"Saved",c:"#ef4444"}];
                })().map(s=>(
            ...
            {allResults
              ...
              .map((j,idx)=>(<JobRow key={idx} job={j} idx={idx} saved={saved} toggleSave={toggleSave} showKeyword applyStatus={applyStatus} onApply={applyJob} backendOnline={backendOnline} notes={notes} onNoteChange={saveNote} editNoteKey={editNoteKey} setEditNoteKey={setEditNoteKey} />))}
            <div style={{ textAlign:"center", marginTop:10 }}>
              <button onClick={()=>{ setAllResults([]); setResults([]); setSaved(new Set()); }} style={{ padding:"6px 16px", fontSize:9.5, fontWeight:600, borderRadius:5, background:"#090d1a", color:"#1e293b", border:"1px solid #0e1320", fontFamily:"var(--mono)" }}>Clear All</button>
            </div>
          </div>)}
        </div>)}
        ...
        {tab === "apply" && (<div>
          ...
          {backendOnline && (
            <div style={{ background:"#090d1a", border:"1px solid #111827", borderRadius:7, padding:"8px 10px", marginBottom:10 }}>
              <div style={{ fontSize:9, fontWeight:700, color:"#3a4660", fontFamily:"var(--mono)", marginBottom:6, letterSpacing:1 }}>PLATFORM SESSIONS</div>
              <div style={{ display:"flex", gap:6, flexWrap:"wrap" }}>
                {["linkedin","naukri","indeed","glassdoor"].map(p => {
                  const s = sessionStatus[p];
                  const logged = s?.saved;
                  return (
                    <div key={p} style={{ display:"flex", alignItems:"center", gap:5, background: logged?"#10b98108":"#ef444408",
                      border:"1px solid "+(logged?"#10b98120":"#ef444418"), borderRadius:5, padding:"4px 8px" }}>
                      <div style={{ width:6, height:6, borderRadius:"50%", background: logged?"#10b981":"#ef4444" }} />
        ...
        {/* ===== PROFILE TAB ===== */}
        {tab === "profile" && (<div>
          <div style={{ background:"#090d1a", border:"1px solid #111827", borderRadius:8, padding:"12px 14px", marginBottom:10 }}>
            <div style={{ fontSize:9, fontWeight:700, color:"#3a4660", fontFamily:"var(--mono)", letterSpacing:1, marginBottom:10 }}>CANDIDATE PROFILE (overrides defaults when applying)</div>
            {[
              { label:"Full Name",         key:"name",          ph:"Gautam Kumar" },
              { label:"First Name",        key:"firstName",     ph:"Gautam" },
              { label:"Last Name",         key:"lastName",      ph:"Kumar" },
              { label:"Email",             key:"email",         ph:"gkt2work@gmail.com" },
              { label:"Phone",             key:"phone",         ph:"9934455873" },
              { label:"City",              key:"city",          ph:"Bangalore" },
              { label:"Total Exp (yrs)",   key:"totalExpYears", ph:"1" },
              { label:"Current CTC (LPA)", key:"currentCTC",   ph:"5" },
              { label:"Expected CTC (LPA)",key:"expectedCTC",  ph:"10" },
              { label:"Notice Period",     key:"noticePeriod",  ph:"30 days" },
              { label:"LinkedIn URL",      key:"linkedin",      ph:"https://linkedin.com/in/..." },
              { label:"GitHub/Portfolio",  key:"github",        ph:"https://github.com/..." },
            ].map(f => (
              <div key={f.key} style={{ marginBottom:8 }}>
                <label style={{ fontSize:8.5, fontWeight:700, color:"#64748b", fontFamily:"var(--mono)", display:"block", marginBottom:3 }}>{f.label}</label>
                <input type="text" value={profileEdit[f.key] || ""} placeholder={f.ph}
                  onChange={e => saveProfile({ [f.key]: e.target.value })}
                  style={{ width:"100%", padding:"6px 9px", fontSize:11, borderRadius:6, background:"#060810", border:"1px solid #1e293b", color:"#e2e8f0", fontFamily:"var(--mono)" }} />
              </div>
            ))}
            <div style={{ display:"flex", gap:6, marginTop:8 }}>
              <button onClick={() => { localStorage.removeItem("vj_profile"); setProfileEdit({}); }} style={{
          ...
          {/* CV Profiles (multiple resumes) */}
          <div style={{ background:"#090d1a", border:"1px solid #111827", borderRadius:8, padding:"12px 14px" }}>
            <div style={{ fontSize:9, fontWeight:700, color:"#3a4660", fontFamily:"var(--mono)", letterSpacing:1, marginBottom:8 }}>CV PROFILES</div>
            {cvProfiles.length === 0 && (
              <div style={{ fontSize:9.5, color:"#1e293b", fontFamily:"var(--mono)", marginBottom:8 }}>No custom CVs yet. Upload a PDF to create one.</div>
            )}
            {cvProfiles.map((cv, i) => (
              <div key={i} style={{ display:"flex", alignItems:"center", gap:6,
                background: i===activeCvIdx?"#6366f110":"#060810",
                border:"1px solid "+(i===activeCvIdx?"#6366f130":"#111827"),
                borderRadius:5, padding:"6px 10px", marginBottom:4 }}>
                <div style={{ fontSize:8, background:"#ef444420", color:"#ef4444", borderRadius:3, padding:"1px 5px", fontFamily:"var(--mono)", fontWeight:700 }}>PDF</div>
                <span style={{ flex:1, fontSize:10, fontWeight:600, color:"#e2e8f0" }}>{cv.name}</span>
                <button onClick={() => setActiveCvIdx(i)} style={{ fontSize:8, fontWeight:700, padding:"2px 7px", borderRadius:3,
                  background: i===activeCvIdx ? "#6366f130" : "#090d1a", color: i===activeCvIdx ? "#818cf8" : "#3a4660",
                  border:"1px solid "+(i===activeCvIdx?"#6366f140":"#1e293b"), fontFamily:"var(--mono)" }}>
                  {i===activeCvIdx ? "Active" : "Set Active"}
                </button>
                <button onClick={() => {
                  const updated = cvProfiles.filter((_,j) => j!==i);
                  setCvProfiles(updated);
                  localStorage.setItem("vj_cv_profiles", JSON.stringify(updated));
                  if (activeCvIdx >= updated.length) setActiveCvIdx(0);
                }} style={{ background:"none", border:"none", color:"#1e293b", cursor:"pointer", fontSize:11 }}>✕</button>
              </div>
            ))}
            <label style={{ display:"flex", alignItems:"center", gap:6, padding:"7px 12px", borderRadius:6,
              border:"1px dashed #1e293b", cursor:"pointer", fontSize:9.5, color:"#3a4660", fontFamily:"var(--mono)", marginTop:4 }}>
              ...
              <input type="file" accept=".pdf" style={{ display:"none" }} onChange={e => {
                ...
                reader.onload = ev => {
                  ...
                  const updated = [...cvProfiles, { name, base64: ev.target.result }];
                  setCvProfiles(updated);
                  localStorage.setItem("vj_cv_profiles", JSON.stringify(updated));
                  setActiveCvIdx(updated.length - 1);
                };
      ...
      {clModal && (
        <CoverLetterModal
          job={clModal.job}
          initialCL={clModal.coverLetter}
          onConfirm={cl => { setClModal(null); clModal.resolve(cl); }}
          onCancel={() => { setClModal(null); clModal.resolve(null); }}
        />
      )}
    </div>
  );
...
function JobRow({ job, idx, saved, toggleSave, showKeyword, applyStatus, onApply, backendOnline, notes, onNoteChange, editNoteKey, setEditNoteKey }) {
  const key = job.url || (job.title + job.company);
  const status = applyStatus?.[key];
  const note = notes?.[key] || "";
  const noteOpen = editNoteKey === key;
  const ageColor = getAgeColor(job.posted);
  const srcColor = { LinkedIn:"#0A66C2", Naukri:"#4A90D9", Indeed:"#2557a7", "Google Jobs":"#4285F4", Glassdoor:"#0caa41" }[job.source] || "#818cf8";
    ...
    <div className="row" style={{
      ...
      <div style={{ display:"flex", alignItems:"flex-start", gap:7 }}>
        <div style={{ flex:1, minWidth:0 }}>
          ...
          <div style={{ fontSize:9.5, color:"#2d3a54", marginBottom:3, display:"flex", gap:5, flexWrap:"wrap", alignItems:"center" }}>
            <span style={{ fontWeight:600, color:"#4b5c78" }}>{job.company}</span>
            <span style={{ color:"#0e1320" }}>|</span>
            <span>{job.location}</span>
            <span style={{ color:"#0e1320" }}>|</span>
            <span style={{ color: ageColor, fontWeight: ageColor !== "#3a4660" ? 600 : 400 }}>{job.posted}</span>
            {job.exp != null && <><span style={{ color:"#0e1320" }}>|</span><span style={{ fontSize:8.5, fontWeight:700, padding:"1px 5px", borderRadius:3, background:"#eab30815", color:"#eab308", fontFamily:"var(--mono)" }}>{job.exp}yr</span></>}
            ...
            {job.source && <span style={{ fontSize:8.5, fontWeight:700, padding:"1px 5px", borderRadius:3, background:srcColor+"15", color:srcColor, fontFamily:"var(--mono)" }}>{job.source}</span>}
          </div>
          ...
          {noteOpen && (
            <textarea rows={2} value={note}
              ...
              autoFocus
              style={{ marginTop:5, width:"100%", background:"#060810", border:"1px solid #6366f130", color:"#c4b5fd",
                borderRadius:5, padding:"5px 8px", fontSize:9.5, fontFamily:"var(--sans)", resize:"vertical" }} />
          )}
          {note && !noteOpen && (
            <div style={{ marginTop:4, fontSize:9, color:"#818cf8", fontFamily:"var(--mono)", fontStyle:"italic" }}>📝 {note.slice(0,80)}{note.length>80?"…":""}</div>
          )}
        </div>
        <div style={{ display:"flex", gap:3, flexShrink:0, alignItems:"flex-start" }}>
          ...
          <button onClick={() => setEditNoteKey?.(noteOpen ? null : key)} title={note ? "Edit note" : "Add note"} style={{
            background: noteOpen ? "#6366f115" : note ? "#6366f110" : "#060810",
            border:"1px solid "+(noteOpen||note?"#6366f130":"#0e1320"),
            borderRadius:4, width:26, height:26, fontSize:10,
            display:"flex", alignItems:"center", justifyContent:"center",
            color: noteOpen||note ? "#818cf8" : "#1e293b"
          }}>✏</button>
